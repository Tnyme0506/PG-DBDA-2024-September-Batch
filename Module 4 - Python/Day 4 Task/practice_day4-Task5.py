...
def coder(inp_string):
    lst_string = inp_string.split(" ")
    new_sent = []
    coded_string = []
    ...
    new_sent = ["".join(key[letter] if letter in key else letter for letter in word) for word in lst_string]

    coded_string = " ".join(new_sent)
    print(coded_string)
